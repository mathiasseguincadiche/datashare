...
describe('FilesController', () => {
  ...
  describe('uploadFile', () => {
    it('should delegate to filesService.uploadFile', () => {
      ...
      const result = controller.uploadFile(file, mockUser as any, options as any);

      expect(filesService.uploadFile).toHaveBeenCalledWith(file, mockUser.id, options);
  ...
  describe('downloadFile', () => {
    it('should call filesService.downloadFile and stream the file', async () => {
      const fileData = { filePath: '/uploads/uuid.pdf', originalName: 'rapport.pdf', mimeType: 'application/pdf' };
      ...
      expect(filesService.downloadFile).toHaveBeenCalledWith('abc-def', 'secret123');
  ...
  describe('deleteFile', () => {
    it('should delegate to filesService.deleteFile with id and user id', () => {
      ...
      expect(filesService.deleteFile).toHaveBeenCalledWith('file-uuid-456', mockUser.id);
    });
  });
});
